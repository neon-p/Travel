...
<?php
	if(isset($_POST['Add'])){
		...
		$target_dir="PDF/";
		$target_file= $target_dir.basename($_FILES["place_details"]["name"]);

		$query="INSERT INTO `place`(`place_name`, `place_details`,`city_id`) VALUES ('$place_name','$path','$city_id')";
		if (mysqli_query($con, $query)){
      $_SESSION["city_id"]=$city_id;
			move_uploaded_file($_FILES["place_details"]["tmp_name"], $target_file);
			header("location:admin_add_place_image.php");
		}else{
			echo"Data Not Saved";
		}
	}
      ...
      <div class="row">
        <div class="col-xs-12 col-sm-8 col-md-6 col-sm-offset-2 col-md-offset-3">
          <div class="about-heading">
			<h1>Place Details</h1>
			<br>
			<form class="form" method="post" action="admin_add_place.php" enctype="multipart/form-data" style="margin-left: 22%">
							  ...
							  <div class="">
							  	<div class="form-group">
							  		<button type="submit" class="btn btn-primary" style="border-radius:5px;margin-left: 37%" name="Add">Add</button>
							  	</div>
							  </div>
							</form>
          </div>
